...
Directory:
    ...
    2.随机旋转 rotate
    3.对比度变换 contrast
    4.gamma变换 gamma_trans
    5.图像噪声 imgnoise
    6.PCA变换 PCA Jittering
    7.高斯边缘模糊 gauss_edge  用不到
    8.高斯模糊 Gauss
    9.图像瑞化 sharp
    10.HSV变换 HSV_trans
    11. RGB变换 RGB_trans
    appendex: 前景分割 BinarySP ramdomcrop
"""
import os
import numpy as np
from numpy import * 
...
def randomCrop(img, input_size):
    '''
    randomcrop 默认(h,w,c)
    '''
    assert img.shape[0] >= input_size
    assert img.shape[1] >= input_size

    x = random.randint(0, img.shape[0] - input_size)
    y = random.randint(0, img.shape[1] - input_size)
