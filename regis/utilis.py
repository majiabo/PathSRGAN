...
def get_coors_from_csv(csv_path,slide_id,target_20x = 768,CLASS = ['HSIL', 'HISL', 'Hisl', "2130", "2140",
                                                                   'LSIL', 'LISL', 'Lisl', "2120",
                                                                   'ASCUS', 'ASC-US', 'ASC-H', "2111", "2112", '3000', 
                                                                   'AGC', 'Agc', "2211", "2212", "2213", "2221", 
                                                                   "2222", "2230", "2241", "2242","null","bubble"]):
    """
    lisubo所写，majiabo改写
    从csv文件中获取坐标信息
    ...
    for c in CLASS:
            listIndex += [csv2.index(line) for line in csv2 if c in line.strip().split(',')[0]]
            info[c] = []
    ...
    for lr_x, lr_y in zip(ran_xs, ran_ys):
        ...
        if max_shift <= 9000:
            ...
            if val < 0.6:
                ...
                print('maxVal:', val)
        elif max_shift > 9000 and max_shift <= 15000:
            # 取上方区域
            try:
                hr_sample = np.array(
                    hr.read_region((hr_x, hr_y), level=0, size=(hsize, max_shift - 9000 + int(lsize * ratio))).convert(
                        'RGB'))
            ...
            try:
                hr_sample = np.array(
                    hr.read_region((hr_x, hr_y), level=0, size=(max_shift - 9000 + int(lsize * ratio), hsize)).convert(
                        'RGB'))
            except:
                return None, None, None, None, False
            result = cv2.matchTemplate(hr_sample, lr_sample, cv2.TM_CCOEFF_NORMED)
            ...
            try:
                hr_sample = np.array(
                    hr.read_region((hr_x + hsize - max_shift + 9000 - int(lsize * ratio), hr_y), level=0,
                                   size=(max_shift - 9000 + int(lsize * ratio), hsize)).convert('RGB'))
            except:
                return None, None, None, None, False
            result = cv2.matchTemplate(hr_sample, lr_sample, cv2.TM_CCOEFF_NORMED)
            _, val, _, maxloc = cv2.minMaxLoc(result)
            if val >= 0.6:
                dW.append(maxloc[0] + hsize + 9000 - int(lsize * ratio) - 2 * max_shift)
                dH.append(maxloc[1] - max_shift)
                print('maxVal:', val)
                continue
            ...
            try:
                hr_sample = np.array(
                    hr.read_region((hr_x, hr_y + hsize - max_shift + 9000 - int(lsize * ratio)), level=0,
                                   size=(hsize, max_shift - 9000 + int(lsize * ratio))).convert('RGB'))
            except:
                return None, None, None, None, False
            result = cv2.matchTemplate(hr_sample, lr_sample, cv2.TM_CCOEFF_NORMED)
            _, val, _, maxloc = cv2.minMaxLoc(result)
            if val >= 0.6:
                dW.append(maxloc[0] - max_shift)
                dH.append(maxloc[1] + hsize + 9000 - int(lsize * ratio) - 2 * max_shift)
                print('maxVal:', val)
                continue
            else:
                max_shift += 1000
...
def fast_regis_coarse(low_path, high_path, slide_ratio, max_shift = 16000, points = 2):
    """
        粗配函数
        path:
            图片路径
        ...
        points：匹配时采样的点数
        返回：平均坐标偏移平均值及各样本值
        down sample input ,so this function could speed up regis
        """
    # do not change this, i don't test lower
    level = 1
    lsize = 500
    i = 0
    ratio = slide_ratio[1] / slide_ratio[0]
    ...
    w, h = lr.dimensions
    safelength = int(w // 3)
    ran_xs = np.random.randint(safelength, high=w - safelength, size=10)
    ran_ys = np.random.randint(safelength, high=h - safelength, size=10)

    for lr_x, lr_y in zip(ran_xs, ran_ys):
        if len(dW) >= points:
            break
        i += 1
        lr_sample = np.array(lr.read_region((lr_x, lr_y), level=level, size=(lsize, lsize)).convert('RGB'))
        lr_sample = cv2.resize(lr_sample, (0, 0), fx=ratio, fy=ratio)
        # 将低倍图坐标映射到高倍图
        hr_x = int(lr_x * ratio - max_shift)
        hr_y = int(lr_y * ratio - max_shift)
        hsize = int(lsize * ratio + max_shift // hr.level_downsamples[1]*2)

        hr_sample = np.array(hr.read_region((hr_x, hr_y), level=level, size=(hsize, hsize)).convert('RGB'))

        result = cv2.matchTemplate(hr_sample, lr_sample, cv2.TM_CCOEFF_NORMED)
        _, val, _, maxloc = cv2.minMaxLoc(result)
        if val > 0.6:
            dW.append((maxloc[0] - max_shift//hr.level_downsamples[1])*hr.level_downsamples[1])
            dH.append((maxloc[1] - max_shift//hr.level_downsamples[1])*hr.level_downsamples[1])
            print('maxVal:', val)
    if len(dW) == 0:
        print('Match failed !!!')
        return None, None, None, None, False
    ex = int(sum(dW)/len(dH))
    ey = int(sum(dH)/len(dH))


    return ex, ey, dW, dH, True
...
"""def generate_neg_from_posi(slide_id, svs_dir, imgBin_dir, labelmap_dir, target_20x=512):
    
    从阳性图中随机取阴性图，返回20倍坐标
    
    imgBin_path = os.path.join(imgBin_dir,slide_id)+'.tif'
    labelmap_path = os.path.join(labelmap_dir,slide_id)+'.tif'
    svs_path = os.path.join(svs_dir,slide_id)+'.svs'
    imgBin = cv2.imread(imgBin_path)
